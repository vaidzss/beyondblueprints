...
initial="hidden"
    ...
    className="mt-12 h-full mb-4">
      <h2 className="text-5xl font-bold text-center font-libre text-[#310e10]">Projects</h2>
      <div className="grid lg:grid-cols-3 md:grid-cols-2 gap-4 p-8">
        {Object.entries(projectPhotos).map(([projectId, photos]) => (
          <div key={projectId} onClick={() => openDialog(projectId)} className="cursor-pointer bg-slate-100 rounded-md p-4 shadow-md">
            <img src={photos[0]?.url} alt={photos[0]?.title} className="rounded shadow-md max-h-fit object-cover h-56 w-full" />
            <p className="mt-2 font-semibold font-playfair">{photos[0]?.title}</p>
          </div>
        ))}
      </div>
  ...
  <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
    <div className="bg-white p-6 rounded-lg max-w-4xl w-full relative max-h-[90vh] overflow-hidden">
      {/* Close button */}
      <button
        onClick={closeDialog}
      ...
      <div className="overflow-y-auto max-h-[70vh] pr-2">
        <div className="space-y-4">
          ...
          <p className="font-poppins text-[#6f4d38] text-center">&ldquo;{projectPhotos[activeProject]?.[0]?.description}&rdquo;</p>
        </div>
      </div>
    </div>
  </div>
